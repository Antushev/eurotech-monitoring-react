...
const PopupAddLink = (props) => {
  ...
  const [parseData, setParseData] = useState({price: stats.stat.price, count: stats.stat.count});
  ...
  const inputRef = useRef(stats.stat.linkProduct);
  ...
  useEffect(() => {
    inputRef.current.value = stats.stat.linkProduct;

    const {price, count, priceSelector, countSelector} = stats.stat;

    if (priceSelector) {
      setParseData({ price: price, count: count });
      ...
      <div className="modal">
        ...
        <ul className="info-list info-list--margin-bottom">
          <li className="info-list__item">
            ...
            <span>
              <span className="info-list__link-text">Ссылка на товар:</span>
          ...
          <li className="info-list__item">
            ...
            <span>
              <span className="info-list__link-text">Название фирмы конкурента:</span>
          ...
          <li className="info-list__item">
            ...
            <span>
              <span className="info-list__link-text">Текущая ссылка на товар конкурента:</span>
              <a href={`${stats.stat.linkProduct}`} target="_blank">Перейти</a>
            </span>
          </li>
        </ul>

        <div className="message message--warning message--no-margin-top message--margin-bottom">
          ...
          <span className="message__text">
            <b>ВНИМАНИЕ!</b> После замены ссылки на новую или перевыбора данных при помощи расширения, все данные, извлечённые по предыдущей ссылке, будут удалены!
          </span>
        </div>
        ...
        >
          {
            isLoadLink || isLoadProducts ? <Preloader /> : 'Редактировать ссылку'
          }
        </button>
      </div>
    </>
